...
class GenerationInput:
  ...
  def for_model(cls, model_name: str, **attrs: t.Any) -> type[GenerationInput]:
    ...
    return attr.make_class(
        ...
        attrs={"prompt": attr.field(type=str), "llm_config": attr.field(type=llm_config.__class__, default=llm_config, converter=functools.partial(cls.convert_llm_config, cls=llm_config.__class__))})
...
def unmarshal_vllm_outputs(request_output: vllm.RequestOutput) -> DictStrAny:
  return dict(
      ...
      outputs=[dict(index=it.index, text=it.text, token_ids=it.token_ids, cumulative_logprob=it.cumulative_logprob, logprobs=it.logprobs, finish_reason=it.finish_reason) for it in request_output.outputs],
  )
