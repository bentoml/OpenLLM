...
class SamplingParams(ReprMixin):
  ...
  def build(self) -> vllm.SamplingParams: return vllm.SamplingParams(max_tokens=self.max_tokens, temperature=self.temperature, top_k=self.top_k, top_p=self.top_p, **converter.unstructure(self))
  ...
  def from_generation_config(cls, generation_config: GenerationConfig, **attrs: t.Any) -> Self:
    ...
    return cls(_internal=True, temperature=temperature, top_k=top_k, top_p=top_p, max_tokens=max_tokens, repetition_penalty=repetition_penalty, length_penalty=length_penalty, early_stopping=early_stopping, **attrs)
...
class LLMConfig(_ConfigAttr):
  ...
  def to_sampling_config(self) -> vllm.SamplingParams: return self.sampling_config.build()
