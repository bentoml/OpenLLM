...
def structure_settings(cl_: type[LLMConfig], cls: type[_ModelSettingsAttr]) -> _ModelSettingsAttr:
  ...
  if _conversation is None:
    _conversation = dict(system_message='', roles=('', ''), sep_style=SeparatorStyle.NO_COLON_SINGLE, sep='')
  ...
  if _fine_tune_strategies is not None:
    ...
    for _possible_ft_config in _fine_tune_strategies:
      ...
      _converted[_adapter_type] = FineTuneConfig(
        PeftType[_adapter_type], _possible_ft_config, False, _llm_config_class
      )
...
class LLMConfig(_ConfigAttr[t.Any, t.Any]):
  ...
  def get_conversation_template(self) -> Conversation:
    ...
    if hasattr(self, 'default_system_message'):
      template.set_system_message(self.default_system_message)
    return template
