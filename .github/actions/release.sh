...
release_package() {
    ...
    jq --arg release_version "${version}" '.version = $release_version' < package.json > package.json.tmp && mv package.json.tmp package.json
    popd &>/dev/null
    pushd external/clojure &>/dev/null
    jq --arg release_version "${version}" '.version = $release_version' < package.json > package.json.tmp && mv package.json.tmp package.json
    popd &>/dev/null
    towncrier build --yes --version "${version}"
    cp CHANGELOG.md openllm-python/CHANGELOG.md
    git add CHANGELOG.md openllm-python/CHANGELOG.md changelog.d package.json openllm-node/package.json external/clojure/package.json
