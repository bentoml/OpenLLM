...
def mount_to_svc(svc: bentoml.Service, llm_runner: openllm.LLMRunner) -> bentoml.Service:
  app = Starlette(debug=True,
                  routes=[
                      Route('/v1/models', functools.partial(list_models, llm_runner=llm_runner), methods=['GET']),
                      Route('/v1/completions', functools.partial(create_completions, llm_runner=llm_runner), methods=['POST']),
                      Route('/v1/chat/completions', functools.partial(create_chat_completions, llm_runner=llm_runner), methods=['POST'])
                  ])
...
# POST /v1/chat/completions
async def create_completions(req: Request, llm_runner: openllm.LLMRunner) -> Response:
  ...
