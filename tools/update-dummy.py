...
from openllm_core._configuration import LiteralBackend
...
_dependencies: dict[LiteralBackend, str] = {
    k: v for k, v in zip(LiteralBackend.__args__, ('torch', 'tensorflow', 'flax', 'vllm'))
}
_auto: dict[str, str] = {
    k: v for k, v in zip(LiteralBackend.__args__, ('AutoLLM', 'AutoTFLLM', 'AutoFlaxLLM', 'AutoVLLM'))
}

def get_target_dummy_file(framework: LiteralBackend) -> Path:
  return _ROOT / 'openllm-python' / 'src' / 'openllm' / 'utils' / f'dummy_{framework}_objects.py'

def mapping_names(framework: LiteralBackend):
  return 'MODEL_MAPPING_NAMES' if framework == 'pt' else f'MODEL_{framework.upper()}_MAPPING_NAMES'

def get_mapping(framework: LiteralBackend) -> OrderedDict[t.Any, t.Any]:
  return getattr(auto, mapping_names(framework))

def make_class_stub(model_name: str, framework: LiteralBackend, indentation: int = 2, auto: bool = False) -> list[str]:
  _dep_list: list[str] = [
      f'"{v}"' for v in [
...
def write_stub(framework: LiteralBackend, _path: str) -> list[str]:
  base = [
